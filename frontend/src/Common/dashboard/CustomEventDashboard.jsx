...
const CustomEventDashboard = () => {
    ...
    useEffect(() => {
        const fetchCustomEvents = async () => {
            try {
                const res = await axiosPublic.get('/custom-event');
                setCustomEvent(res.data);
            } catch (error) {
    ...
    const handleApprove = async (id) => {
        try {
            const res = await axiosPublic.patch(`/custom-event/${id}`);
            if (res.data.status === 'pending') {
                setCustomEvent(prevEvents =>
                    prevEvents.map(event => event._id === id ? { ...event, status: 'approved' } : event)
                );
                toast.success('Request approved');
            }
        } catch (error) {
    ...
    const handleReject = async (id) => {
        try {
            const res = await axiosPublic.put(`/custom-event/${id}`);
            if (res.data.status === 'pending') {
                setCustomEvent(prevEvents =>
                    prevEvents.map(event => event._id === id ? { ...event, status: 'rejected' } : event)
                );
                toast.success('Request rejected');
            }
        } catch (error) {
                            ...
                            <tr key={event._id} className='text-center'>
                                ...
                                <td className="px-4 py-2">
                                    {event.status === 'pending' ? (
                                        ...
                                        <p className={`text-white bg-gradient-to-r ${event.status === 'approved' ? 'from-green-600 via-green-700 to-green-600' : 'from-green-400 via-green-500 to-green-600'} hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 ${event.status === 'approved' ? '' : 'opacity-60'}`}>
                                ...
                                <td className="px-4 py-2">
                                    {event.status === 'pending' ? (
                                        ...
                                        <p className={`text-white bg-gradient-to-r ${event.status === 'rejected' ? 'from-red-400 via-red-500 to-red-600' : 'from-red-400 via-red-500 to-red-600'} hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 ${event.status === 'rejected' ? '' : 'opacity-60'}`}>
